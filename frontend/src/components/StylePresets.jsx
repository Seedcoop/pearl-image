...
const StylePresets = ({ currentAssetType, selectedPreset, onPresetSelect }) => {
  ...
  return (
    <section>
      <h3 className="text-pearl-text text-base sm:text-lg font-bold leading-tight tracking-[-0.015em] mb-3">스타일 프리셋</h3>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 gap-2 sm:gap-3">
        {Object.entries(presets).map(([key, preset]) => (
          <div
            key={key}
            className={`p-3 sm:p-2 rounded-lg border-2 transition-all duration-300 cursor-pointer group ${
